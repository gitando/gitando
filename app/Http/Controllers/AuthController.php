...
class AuthController extends Controller
{
    ...
    public function appLogin(Request $request)
    {
        ...
        if (!$user) {
            return response()->json([
                'message' => __('gitando.invalid_login_message'),
                'errors' => __('gitando.invalid_login')
            ], 401);
        }
    ...
    */
    public function login(Request $request)
    {
        ...
        $user->jwt = JWT::encode(['iat' => time(),'exp' => time() + config('gitando.jwt_refresh')], config('gitando.jwt_secret').'-Rfs');
        $user->save();

        return response()->json([
            'access_token' => JWT::encode(['iat' => time(),'exp' => time() + config('gitando.jwt_access')], config('gitando.jwt_secret').'-Acs'),
    ...
    */
    public function refresh(Request $request)
    {
        ...
        if ($user) {
            ...
            return response()->json([
                'message' => __('gitando.invalid_token_message'),
                'errors' => __('gitando.invalid_token')
            ], 401);
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->newusername) {
            ...
            if (!Auth::where('username', $newuser)->first()) {
                ...
                return response()->json([
                    'message' => __('gitando.username_conflict_message'),
                    'errors' => __('gitando.username_conflict')
                ], 409);
            }
        }
