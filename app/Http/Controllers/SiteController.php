...
class SiteController extends Controller
{
    ...
    */
    ...
    public function create(Request $request)
    {
        ...
        if ($request->php) {
            if (!in_array($request->php, config('gitando.phpvers'))) {
                return response()->json([
                    'message' => __('gitando.bad_request'),
                    'errors' => __('gitando.invalid_php_version')
                ], 400);
            }
            $php = $request->php;
        } else {
            $php = config('gitando.default_php');
        }
        ...
        if ($conflict) {
            return response()->json([
                'message' => __('gitando.site_domain_conflict_message'),
                'errors' => __('gitando.site_domain_conflict')
            ], 409);
        }
        ...
        $site->username   = config('gitando.users_prefix').hash('crc32', (Str::uuid()->toString())).rand(1, 9);
     ...
     *                  type="string",
     *                  example="andreapollastri/gitando",
     *             ),
     *            @OA\Property(
     *                  property="branch",
            ...
            if ($site->domain != strtolower($request->domain)) {
                ...
                foreach ($sites as $checksite) {
                    if (strtolower($request->domain) == $checksite->domain) {
                        return response()->json([
                            'message' => __('gitando.server_conflict_domain_message'),
                    ...
                    foreach ($checksite->aliases as $alias) {
                        if (strtolower($request->domain) == $alias->domain) {
                            return response()->json([
                                'message' => __('gitando.server_conflict_alias_message'),
                                'errors' => __('gitando.server_conflict')
                            ], 409);
                        }
                    }
    ...
    */
    public function destroy(string $site_id)
    {
        ...
        if ($site->panel) {
            return response()->json([
                'message' => __('gitando.bad_request_default_site_delete'),
    ...
    */
    ...
    {
        try {
            JWT::decode($pdftoken, config('gitando.jwt_secret').'-Pdf', ['HS256']);
        } catch (\Throwable $th) {
            abort(403);
        }
    ...
    */
    public function destroyalias(string $site_id, string $alias_id)
    {
        ...
        if (!$alias) {
            return response()->json([
                'message' => __('gitando.alias_not_found_message'),
                'errors' => __('gitando.alias_not_found')
            ], 404);
        }
