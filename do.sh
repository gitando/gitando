...
if [ "$ID" = "ubuntu" ]; then
    case $VERSION in
        ...
        *)
            ...
            echo "Gitando requires Linux Ubuntu 22.04 LTS"
            echo "${reset}"
            exit 1;
            break
            ;;
...
sudo rpl -i -w "GITANDOSERVERID" $SERVERID /var/www/html/database/seeders/DatabaseSeeder.php
