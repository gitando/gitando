    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('gitando.change_username') }}
            </div>
            <div class="card-body">
                <p>{{ __('gitando.current_username') }}: <b><span id="currentuser"></span></b></p>
                <div class="input-group">
                    <input class="form-control" type="text" placeholder="{{ __('gitando.change_username_placeholder') }}" id="newuser" autocomplete="off" />
    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('gitando.panel_url') }}
            </div>
            <div class="card-body">
                <p>{{ __('gitando.panel_url_text') }}</p>
                <div class="input-group">
                    <input class="form-control" type="text" placeholder="panel.domain.ltd" id="panelurl" autocomplete="off" />
                    <div class="input-group-append">
                        <button class="btn btn-warning" type="button" id="panelurlssl" data-toggle="tooltip" data-placement="top" title="{{ __('gitando.panel_url_force_ssl') }}"><i class="fas fa-lock"></i></button>
                    </div>
                    <div class="input-group-append">
    ...
    <div class="col-xl-6">
        <div class="card mb-4">
            <div class="card-header">
                ...
                {{ __('gitando.panel_api') }}
            </div>
            <div class="card-body">
                <p>{{ __('gitando.panel_api_endpoint') }}:  <b>{{ URL::to('/api/') }}</b></p>
                <div class="text-center">
                    <button class="btn btn-primary mr-3" type="button" id="newapikey">{{ __('gitando.renew_api_key') }}</button>
                    <a href="/api/docs" target="_blank">
                        <button class="btn btn-warning" type="button">{{ __('gitando.documentation') }}</button>
                    </a>
                </div>
                <div class="space"></div>
            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="authorizeModalLabel">{{ __('gitando.action_authorization_modal_title') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p>{{ __('gitando.action_authorization_modal_text') }}:</p>
                <div class="input-group">
                    <input class="form-control" type="password" id="currentpass" />
                </div>
                <div class="space"></div>
                <div class="text-center">
                    <button class="btn btn-primary" type="button" id="submit">{{ __('gitando.submit') }} <i class="fas fa-circle-notch fa-spin d-none" id="loading"></i></button>
                </div>
                <div class="space"></div>
            </div>
        </div>
        ...
        if(!oldpassval || oldpassval.length < 8) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(patchcall == 'changeuser') {
                        success('{{ __('gitando.username_updated_success') }}');
                        $('#currentuser').html(localStorage.username);
                        $('#username').html(localStorage.username);
                    }
                    if(patchcall == 'changepass') {
                        success('{{ __('gitando.password_updated_success') }}');
                    }
                    if(patchcall == 'newapikey') {
                        success('{{ __('gitando.new_api_key_success') }}:<br><b>'+data.apikey+'</b>');
                    }
